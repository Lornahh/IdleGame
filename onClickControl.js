...
window.onload=function(){
 ...
 let secondUpgradeText = Array.from(document.querySelectorAll("#secondUpgrade"))[0];
...
function firstUpgradeFunction(){
  ...
  x-=10;
  priceof1 = priceof1+(.3*priceof1);
  var priceof1update = priceof1.toFixed(0);
  currencyBox.innerHTML=x;
  let elem = Array.from(document.querySelectorAll("#firstUpgrade"))[0];
...
function secondUpgrade(){
  ...
  x-=priceof1;
  currencyBox.innerHTML=x;
  priceof2 = priceof2+(.3*priceof2);
  var priceof2update = priceof2.toFixed(0);
  ...
  ${priceof2update} and gives you 3 clicks per second.`;
}
else{
  alert("You do not have enough currency!");
}
